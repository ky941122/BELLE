...
class RejectSamplingTrainer:
    ...
    def _inner_training_loop(
        ...
        use_accelerator_prepare = True if self.tmp_model is self.model else False
        ...
        if use_accelerator_prepare:
            ...
            if hasattr(self.lr_scheduler, "step"):
                if self.use_apex:
                    self.tmp_model = self.accelerator.prepare(self.model)
                else:
        ...
        if self.is_fsdp_enabled:
            self.model = self.model_wrapped = self.tmp_model
